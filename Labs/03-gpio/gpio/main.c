...
int main(void)
{
    ...
    while (1)
    {
		...
		//if(bit_is_clear(PIND,BUTTON)) //Stisk tlaèítka, tak LED blikají
		if(!GPIO_read(&PIND,BUTTON)) //!-negace, ~ inverze všech bitù
		{
			GPIO_toggle(&PORTB, LED_GREEN); // Invert Led to Blink
